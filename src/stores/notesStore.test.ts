...
describe('notesStore', () => {
  ...
  describe('createNote', () => {
    it('should create a new note with title and content', () => {
      ...
      const newNote = createNote({ title: 'Test Note', content: '<p>Test content</p>' });
  ...
  describe('getFilteredNotes', () => {
    it('should return all notes sorted by updatedAt when no search query', () => {
      ...
      for (let i = 0; i < filtered.length - 1; i++) {
        expect(filtered[i].updatedAt).toBeGreaterThanOrEqual(filtered[i + 1].updatedAt);
      }
    });
    ...
    it('should filter notes by title', () => {
      ...
      expect(filtered.every((n) => n.title.toLowerCase().includes('welcome'))).toBe(true);
    });
